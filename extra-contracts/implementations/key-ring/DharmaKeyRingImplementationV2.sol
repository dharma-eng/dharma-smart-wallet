...
import "../../../interfaces/DharmaKeyRingImplementationV2Interface.sol";
import "../../../interfaces/ERC1271.sol";


/**
 * @title DharmaKeyRingImplementationV2
 * @author 0age
...
contract DharmaKeyRingImplementationV2 is
  ...
  DharmaKeyRingImplementationV2Interface,
  ERC1271 {
  ...
  uint256 internal constant _DHARMA_KEY_RING_VERSION = 2;
  ...
  function takeAction(
    ...
    _verifyOrderedSignatures(
      ...
      _getStandardActionID(to, value, data, _nonce).toEthSignedMessageHash(),
      signatures
    );
  ...
  function getActionID(
    ...
    actionID = _getStandardActionID(to, value, data, nonce);
  }

  function getNextActionID(
    address payable to, uint256 value, bytes calldata data
  ) external view returns (bytes32 actionID) {
    actionID = _getStandardActionID(to, value, data, _nonce);
  }
  ...
  function getAdminActionID(
    ...
    adminActionID = _getAdminActionID(adminActionType, argument, nonce);
  }

  function getNextAdminActionID(
    AdminActionType adminActionType, uint160 argument
  ) external view returns (bytes32 adminActionID) {
    adminActionID = _getAdminActionID(adminActionType, argument, _nonce);
  }
  ...
  function _getStandardActionID(
    address payable to, uint256 value, bytes memory data, uint256 nonce

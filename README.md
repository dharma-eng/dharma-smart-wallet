...
- Each [Upgrade Beacon Controller](https://github.com/dharma-eng/dharma-smart-wallet/blob/master/contracts/upgradeability/DharmaUpgradeBeaconController.sol) is an ownable contract that emits events related to each upgrade, enforces some conditions as to what upgrades are allowable, and enables the current owner of the controller to transfer ownership to a new owner. (Note that the Upgrade Beacon Controller Manager actually has a known bug still that needs to be fixed, and more tests that need to be written, before it goes into production.)
